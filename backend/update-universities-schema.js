...
async function updateUniversitiesSchema() {
  try {
    console.log('🔄 Updating universities table schema...');
    
    // Drop existing universities table and recreate with new schema
    ...
    // Create new universities table with enhanced schema
    await pool.query(`
    ...
    // Insert enhanced university data
    await pool.query(`
    ...
    // Verify data
    const { rows } = await pool.query('SELECT COUNT(*) as count FROM universities');
    console.log(`🏫 Universities in database: ${rows[0].count}`);
    
    console.log('');
    console.log('🎉 Universities schema updated successfully!');
    console.log('📚 Enhanced features now available:');
    console.log('  - World rankings');
    console.log('  - Program strengths');
    console.log('  - Application deadlines');
    console.log('  - Test requirements');
    console.log('  - Scholarship information');
    console.log('  - Employment statistics');
    console.log('  - Notable alumni');
