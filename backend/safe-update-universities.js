...
async function safeUpdateUniversities() {
  try {
    console.log('🔒 SAFE DATABASE UPDATE - PRESERVING ALL USER DATA');
    console.log('================================================');
    ...
    console.log(`✅ Backed up ${users.length} users and ${profiles.length} profiles`);
    
    // 2. Check if universities table needs updating
    console.log('🔍 Checking universities table structure...');
    const { rows: columns } = await pool.query(`
      SELECT column_name FROM information_schema.columns 
      WHERE table_name = 'universities' AND column_name = 'world_ranking'
    `);
    
    if (columns.length > 0) {
      console.log('✅ Universities table already has enhanced schema - no update needed');
      
      // Verify data
      const { rows: uniCount } = await pool.query('SELECT COUNT(*) as count FROM universities');
      console.log(`🏫 Universities in database: ${uniCount[0].count}`);
      
      console.log('');
      console.log('🎉 Database is already up to date!');
      console.log('👤 User data: PRESERVED ✅');
      console.log('🏫 University data: ENHANCED ✅');
      return;
    }
    
    // 3. Only update universities table (preserve user data)
    console.log('🔄 Updating universities table only...');
    
    // Backup existing shortlists and tasks if any
    const { rows: shortlists } = await pool.query('SELECT * FROM shortlists');
    const { rows: tasks } = await pool.query('SELECT * FROM tasks');
    console.log(`📦 Backed up ${shortlists.length} shortlists and ${tasks.length} tasks`);
    
    // Drop only university-related tables (NOT users or profiles)
    await pool.query('DROP TABLE IF EXISTS shortlists CASCADE');
    await pool.query('DROP TABLE IF EXISTS tasks CASCADE');
    await pool.query('DROP TABLE IF EXISTS universities CASCADE');
    console.log('✅ Old university tables dropped (user data preserved)');
    
    // 4. Create enhanced universities table
    await pool.query(`
    ...
    console.log('✅ Enhanced university tables created');
    
    // 6. Insert enhanced university data
    await pool.query(`
    ...
    console.log('✅ Enhanced university data inserted');
    
    // 7. Verify everything is intact
    const { rows: finalUsers } = await pool.query('SELECT COUNT(*) as count FROM users');
    const { rows: finalProfiles } = await pool.query('SELECT COUNT(*) as count FROM profiles');
    const { rows: finalUniversities } = await pool.query('SELECT COUNT(*) as count FROM universities');
    
    console.log('');
    console.log('🎉 SAFE UPDATE COMPLETED SUCCESSFULLY!');
    console.log('=====================================');
    console.log(`👤 Users preserved: ${finalUsers[0].count} (was ${users.length})`);
    console.log(`📋 Profiles preserved: ${finalProfiles[0].count} (was ${profiles.length})`);
    console.log(`🏫 Universities enhanced: ${finalUniversities[0].count}`);
    console.log('');
    ...
    console.log('');
    console.log('🚨 IF ERROR OCCURRED:');
    console.log('1. User data should still be intact');
    console.log('2. Run: node debug-user.js to verify');
    console.log('3. Contact support if user data is missing');
  }
  
  process.exit(0);
}

safeUpdateUniversities();